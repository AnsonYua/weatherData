...
async function saveForecastCache(payload) {
  ...
  const blob = await writeBlobJson(HKO_CACHE_PATH, payload);
...
async function readForecastCache() {
  ...
  return readBlobJson(HKO_CACHE_PATH, null);
}
...
async function writeSellRules(payload) {
  ...
  await writeBlobJson(SELL_RULES_PATH, payload);
}
...
async function appendSellLog(entries) {
  ...
  let current = { entries: [] };
  try {
    current = await readBlobJson(SELL_LOG_PATH, { entries: [] }, { bypassCache: true });
  } catch {
    current = { entries: [] };
  }
  ...
  await writeBlobJson(SELL_LOG_PATH, payload);
}

async function markRulePending(rulesPayload, ruleId) {
  ...
  await writeSellRules(rulesPayload);
  ...
  await sleep(750);
  const verify = await readBlobJson(
    SELL_RULES_PATH,
    { rules: [] },
    { bypassCache: true, cacheTtlMs: 0 }
  );
  ...
  Object.assign(rulesPayload, verify, { available: true });
  return pendingId;
}

async function updateSellRule(rulesPayload, ruleId, updates) {
  const rules = Array.isArray(rulesPayload.rules) ? rulesPayload.rules : [];
  const rule = rules.find((item) => item.id === ruleId);
...
async function runAutoSellRules({ dryRun }) {
  ...
  for (const rule of rules) {
    ...
    try {
      ...
      const pendingId = await markRulePending(rulesPayload, rule.id);
      if (!pendingId) {
      ...
      await updateSellRule(rulesPayload, rule.id, {
        executed: true,
        pending: false,
        pendingId,
        executedAt: now,
